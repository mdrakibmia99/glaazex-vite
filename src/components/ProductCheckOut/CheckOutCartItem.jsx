...
function CheckOutCartItem({item,index}) {
    ...
    return (<>
        ...
        <div className="flex flex-row box-border items-center gap-[5px] lg:gap-2 lg:px-5  w-full mb-5 justify-between p-5">
            ...
            <div className=" sm:w-1/12 ">
                ...
                <img className="rounded min-w-[50px] h-[50px]" src={item.product_image} alt="" />
            </div>

            <div className="w-4/12">
                <p className="whitespace-nowrap truncate text-base md:text-lg">{item.product_title} </p>
                <p className="text-sm lg:text-base whitespace-nowrap">{item.qty} <span>Items x </span>{item.price} </p>
            </div>

            <div className="w-3/12">
                <span className=" rounded-full whitespace-nowrap text-xs lg:text-base">{item.color}</span>
            </div>
            <div className="3/12">
                <p className="whitespace-nowrap text-sm">selling price</p>
                <p className="text-center text-sm font-bold">{item.price} <span>Tk</span></p>
            </div>

            <div 
             onClick={() => handleDelete(index)}
            className="ml-2 hover:cursor-pointer  hover:bg-slate-400 p-2 rounded-full ease-in duration-500">
